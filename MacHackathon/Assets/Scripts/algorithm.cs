...
{   
    ...
    public void Start(){
		...
		Debug.Log(FinalAns.Date + " Final Answer");
    }
    ...
    public static DateTime MatarikiDay(int year)
    {
        ...
        for (int i = 19; i < 31; i++){ //check all days in june after the 19th
            if (IsCorrectMoonPhase(new DateTime(year, 6, i))){
        ...
        if (Date.Month != 6){
            for (int i = 1; i < 32; i++){ //check all days in july
                if (IsCorrectMoonPhase(new DateTime(year, 7, i))){
    ...
    static bool IsCorrectMoonPhase(DateTime Date)
    {
        ...
        if ( DateTime.IsLeapYear(Date.Year)){
            ...
            if (Date.Month == 7){ //if month is june
                days += Date.Day;
    ...
    {
        Debug.Log(Date.Date + "Tongoraro period Start");
        int counterPlus = 0;
        int counterMinus = 0;

        DateTime TempDatePlus = Date;
        //counting forward
        while ((int) TempDatePlus.DayOfWeek != 5){
